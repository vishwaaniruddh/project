-- Drop existing BOQ tables if they exist
DROP TABLE IF EXISTS `boq_items`;
DROP TABLE IF EXISTS `boq`;
DROP TABLE IF EXISTS `boq_master`;

-- Create the new BOQ items table with proper structure
...
-- Insert all BOQ data from the original table with proper categorization
...
SELECT COUNT(*) as total_items FROM `boq_items`;
SELECT category, COUNT(*) as item_count FROM `boq_items` GROUP BY category ORDER BY category;