...
try {
    ...
    echo "Creating inventory management tables...\n";
    ...
    $db->exec($sql);
    echo "✓ Created inventory_stock table\n";
    
    // 2. Inward Receipts Table
    $sql = "CREATE TABLE IF NOT EXISTS `inventory_inwards` (
      `id` int(11) NOT NULL AUTO_INCREMENT,
    ...
    $db->exec($sql);
    echo "✓ Created inventory_inwards table\n";
    
    // 3. Inward Receipt Items Table
    $sql = "CREATE TABLE IF NOT EXISTS `inventory_inward_items` (
      `id` int(11) NOT NULL AUTO_INCREMENT,
    ...
    $db->exec($sql);
    echo "✓ Created inventory_inward_items table\n";
    
    // 4. Material Dispatches Table
    $sql = "CREATE TABLE IF NOT EXISTS `inventory_dispatches` (
      `id` int(11) NOT NULL AUTO_INCREMENT,
    ...
    $db->exec($sql);
    echo "✓ Created inventory_dispatches table\n";
    
    // 5. Dispatch Items Table
    $sql = "CREATE TABLE IF NOT EXISTS `inventory_dispatch_items` (
      `id` int(11) NOT NULL AUTO_INCREMENT,
    ...
    $db->exec($sql);
    echo "✓ Created inventory_dispatch_items table\n";
    
    // 6. Material Tracking Table
    $sql = "CREATE TABLE IF NOT EXISTS `inventory_tracking` (
      `id` int(11) NOT NULL AUTO_INCREMENT,
      `boq_item_id` int(11) NOT NULL,
      `batch_number` varchar(100) DEFAULT NULL,
    ...
    $db->exec($sql);
    echo "✓ Created inventory_tracking table\n";
    ...
    $db->exec($sql);
    echo "✓ Created inventory_movements table\n";
    
    // 8. Material Reconciliation Table
    $sql = "CREATE TABLE IF NOT EXISTS `inventory_reconciliation` (
      `id` int(11) NOT NULL AUTO_INCREMENT,
    ...
    $db->exec($sql);
    echo "✓ Created inventory_reconciliation table\n";
    
    // 9. Reconciliation Items Table
    $sql = "CREATE TABLE IF NOT EXISTS `inventory_reconciliation_items` (
      `id` int(11) NOT NULL AUTO_INCREMENT,
    ...
    $db->exec($sql);
    echo "✓ Created inventory_reconciliation_items table\n";
    
    // Initialize inventory stock for all existing BOQ items
    $sql = "INSERT IGNORE INTO `inventory_stock` (`boq_item_id`, `current_stock`, `unit_cost`)
            SELECT `id`, 0, 0 FROM `boq_items` WHERE `status` = 'active'";
    
    $db->exec($sql);
    echo "✓ Initialized inventory stock for BOQ items\n";
