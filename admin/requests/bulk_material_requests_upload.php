...
<div class="flex justify-between items-center mb-6">
    <div>
        ...
        <p class="mt-2 text-sm text-gray-700">Upload multiple Material Requests using Excel file</p>
    </div>
    <div class="flex space-x-2">
        <a href="Material_Requests_Format.xlsx" class="btn btn-secondary">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <ul class="text-sm text-gray-600 space-y-1">
                    ...
                    <li>• Required rows: Site ID, Vendor Code, Item Name</li>
                </ul>
            </div>
            ...
            <div>
                ...
                <ol class="text-sm text-gray-600 space-y-1 ordered_list">
                    ...
                    <li>Required Date</li>
                    <li>Request Notes</li>
                    <li>column 2 - Item Name (Required) , column 4 - No of Quantity (Site ID Wise) </li>
                    <li>So on ......</li>
                </ol>
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider max-w-xs">Vendor Code</th>
        ...
        foreach ($sites as $col => $siteId) {
            if($col>=3){
                try{
                    ...
                    $requiredDate  = $data[3][$col] ?? null;
                    $requestNotes  = $data[4][$col] ?? null;
                    $items = [];
                    for ($row = 6; $row < count($data); $row++) {
                        $itemQtys = $data[$row][$col] ?? '';
                        $itemName  = $data[$row][1] ?? null;
                        $_itemName = trim($itemName);
                        $existingItemCodeData = $BoqItemModel->findByItemName($_itemName);
                        $boq_itemId = '';$boq_itemUnit = '';
                        if ($existingItemCodeData) {
                        ...
                        /*
                            if (!$existingItemCodeData) {
                                ...
                                $rowResult['status'] = 'failed';
                                $rowResult['action'] = 'rejected';
                                $rowResult['message'] = "Item Name '{$_itemName}' does not exists in database";
                                $rowResult['errors'][] = 'Item Name does not added in boq items';
                                $results['failed']++;
                                $results['errors'][] = "Col {$col}: Item Name '{$_itemName}' does not exists";
                                $results['rows'][] = $rowResult;
                                continue;
                            }else{
                                $boq_itemId = $existingItemCodeData['id'];
                        ...
                        */
                        $_item_qty = isset($itemQtys) ? (int) trim($itemQtys) : 0;
                        if($_item_qty>0 && $boq_itemId!='' && $boq_itemUnit!=''){
                            $items[] = [
                    ...
                    */
                    ...
                    if(count($items)>0){
                            ...
                            $materialRequestID = $materialRequestModel->create($requestMaterialData);
                    }
                    ...
                    if ($materialRequestID) {
                        ...
                        $rowResult['status'] = 'failed';
                        $rowResult['message'] = 'Failed to create MaterialRequest in database';
                        $rowResult['errors'][] = 'Database insert failed';
                        $results['failed']++;
                        $results['errors'][] = "Col {$col}: Failed to create MaterialRequest for site {$_site_id}";
                    }
