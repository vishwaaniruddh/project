...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $isAjax) {
    ...
    try {
        ...
        $uploadPath = $uploadDir . 'users_' . time() . '_' . $file['name'];
        ...
        $results = processUserExcel($uploadPath);
...
<div class="flex justify-between items-center mb-6">
    <div>
        <h1 class="text-2xl font-semibold text-gray-900">Bulk Upload Users</h1>
        <p class="mt-2 text-sm text-gray-700">Upload multiple Users using Excel file</p>
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <ul class="text-sm text-gray-600 space-y-1">
                    ...
                    <li>• Required columns: Username,email</li>
                </ul>
            </div>
            ...
            <div class="flex items-start">
                ...
                <div>
                    ...
                    <p class="text-sm text-blue-700 mt-1">
                        ...
                        • Existing users will be updated, new users will be created<br>
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-32">Username</th>
        ...
        tableBody.innerHTML = data.rows.map(row => {
            ...
            return `
                <tr class="${row.status === 'failed' ? 'bg-red-50' : ''}">
                    ...
                    <td class="px-3 py-2 whitespace-nowrap text-sm text-gray-900 font-medium">${row.username || '-'}</td>
...
<?php
function processUserExcel($filePath) {
    $results = [
        'success' => true,
        'processed' => 0,
        'created' => 0,
    ...
    try {
        ...
        foreach ($data as $row) {
            ...
            try {
                ...
                // $rowResult['site_id'] = trim($row[0] ?? '');
                // $rowResult['location'] = trim($row[2] ?? '');
                
                // Validate and process row
                $userData = validateAndProcessRow($row, $rowNumber);
                
                if (!empty($userData['errors'])) {
                    $rowResult['status'] = 'failed';
                    $rowResult['errors'] = $userData['errors'];
                    $rowResult['message'] = 'Validation failed: ' . implode(', ', $userData['errors']);
                    $results['rows'][] = $rowResult;
                    $results['failed']++;
                    $results['errors'] = array_merge($results['errors'], $userData['errors']);
                    continue;
                }
                
                // Check if site_id already exists
                // $existingSiteBySiteId = $userModel->findBySiteId($userData['data']['site_id']);
                $existingUserbyUsername = $userModel->findByUsername($userData['data']['username']);
                ...
                if ($existingUserbyUsername) {
                   ...
                   $rowResult['message'] = "User name '{$userData['data']['username']}' already exists in database";
                    $rowResult['errors'][] = 'Duplicate User name';
                    $results['failed']++;
                    // $results['errors'][] = "Row {$rowNumber}: User name '{$data['username']}' already exists";
                    $results['errors'][] = "Row {$rowNumber}: User name '{$userData['data']['username']}' already exists";
               ...
               if (!empty($userData['data']['email'])) {
                    // $existingSiteByStoreId = $userModel->findByStoreId($userData['data']['store_id']);
                     $existingUserByEmail = $userModel->findByEmail($userData['data']['email']);
                    ...
                    if ($existingUserByEmail) {
                        ...
                        $rowResult['status'] = 'failed';
                        $rowResult['action'] = 'rejected';
                        $rowResult['message'] = "Email '{$userData['data']['email']}' already exists in database";
                        $rowResult['errors'][] = 'Duplicate Email';
                        $results['failed']++;
                       $results['errors'][] = "Row {$rowNumber}: Email '{$userData['data']['email']}' already exists";
                        $results['rows'][] = $rowResult;
                        continue;
                    }
                }
                ...
                $user = $userModel->create($userData['data']);
                
                if ($user) {
                    $rowResult['status'] = 'success';
                    $rowResult['message'] = 'User created successfully';
                    $results['created']++;
                } else {
                    $rowResult['status'] = 'failed';
                    $rowResult['message'] = 'Failed to create user in database';
                    $rowResult['errors'][] = 'Database insert failed';
                    $results['failed']++;
                    // $results['errors'][] = "Row {$rowNumber}: Failed to create user {$data['username']}";
                    $results['errors'][] = "Row {$rowNumber}: Failed to create user {$userData['data']['username']}";
 ...
 function validateAndProcessRow($row, $rowNumber)
    {
        ...
        // Expected columns: User name, Email,Phone Number,Password,Role,Status
        $expectedColumns = 6;
        ...
        if (! empty($userData['email'])) {
            if (! filter_var($userData['email'], FILTER_VALIDATE_EMAIL)) {
                $errors[] = "Row {$rowNumber}: Invalid email format for contact person email '{$userData['email']}'";
            }
        }

        // Validate contact person number format (basic validation)
        if (! empty($userData['phone'])) {
            if (! preg_match('/^[\d\s\+\-\(\)]+$/', $userData['phone'])) {
                $errors[] = "Row {$rowNumber}: Invalid phone number format for contact person number '{$userData['phone']}'";
            }
        }
