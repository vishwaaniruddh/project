...
// Decode specifications for display
if (!empty($data['specifications']) && is_string($data['specifications'])) {
    $data['specifications_display'] = json_encode(json_decode($data['specifications'], true), JSON_PRETTY_PRINT);
} else {
    $data['specifications_display'] = '';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($result['success']) {
        ...
        $data['specifications_display'] = $_POST['specifications'] ?? '';
    }
}
        ...
        <form method="POST" class="space-y-6">
            ...
            <div class="form-group">
                ...
                <textarea name="specifications" class="form-textarea font-mono text-sm" rows="4" 
                          placeholder='{"color": "red", "size": "large", "weight": "1.5kg"}'><?php echo htmlspecialchars($data['specifications_display'] ?? ''); ?></textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="form-group">
                    ...
                    <input type="number" name="minimum_stock_level" value="<?php echo intval($data['minimum_stock_level'] ?? 0); ?>" 
